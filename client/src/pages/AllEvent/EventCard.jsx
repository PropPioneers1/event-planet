...
const EventCard = ({ item }) => {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 bg-white border border-primary border-opacity-10 hover:border-opacity-100 rounded transition-all duration-500">
      {/* div for image */}
      <div className="relative w-full md:w-11/12">
        <img
          className="object-cover w-full h-[250px] md:h-full  rounded "
          src={eventImages[0]}
          alt="Event Image"
        />
        {/* date and month*/}
        <div className="absolute top-0 w-[70px] h-[70px] p-6 flex flex-col items-center justify-center  bg-gradient-to-tr  from-primary to-[#921b42]  ">
          <p className="text-2xl text-white font-bold">{eventDate}</p>
