...
const Trendingproducts = () => {
  ...
  return (
    <div className="shadow-bg bg-slate-100 mt-10">
      <Container>
      ...
      <div className="flex ">
        <div>
          <Lottie
            className="md:w-[350px] hidden md:block mt-20"
        ...
        <div className="slider rounded-r-2xl rounded-l-3xl">
          <div className="slide-track gap-5 ">
            {data.concat(data).map((item, index) => (
              <div
                key={index}
                className="slide relative card h-[17rem] bg-transparent "
              >
                <div className="mb-4"></div>
                <div className="trendingimg w-full relative">
                  <img
                    className="h-20 w-full "
                    src={item.image}
                    alt=""
                  />
                  <div className="buy-now-button absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-0 transition-opacity duration-300 hover:opacity-100">
                    <button className="border-2 backdrop-blur-lg font-bold border-slate-700 text-white px-4 py-2 rounded">
                      Buy Now
                    </button>
                  </div>
                </div>
                <div className="h-44 w-56 my-auto">
                  <h2 className="text-slate-700 text-lg text-start my-auto border-slate-300 text-ellipsis font-bold mt-4">
                    {item.itemName}
                  </h2>
                    ...
                    <div className="rating rating-sm mt-1">
                      {Array.from({ length: 5 }, (_, i) => (
                        <input
                          ...
                          type="radio"
                          name={`rating-${item.id}`}
                          className={`mask mask-star-2 bg-blue-950 ${
